...
fn encode_hashmap() {
    ...
    assert_eq!("d3:cow3:moo4:spam4:eggse", map.bencode());

    let map: HashMap<String, Vec<String>> =
        [("spam".to_string(), vec!["a".to_string(), "b".to_string()])]
            .iter()
            .cloned()
            .collect();

    assert_eq!("d4:spaml1:a1:bee", map.bencode());

    let map: HashMap<String, String> = [
        ("publisher".to_string(), "bob".to_string()),
        (
            "publisher-webpage".to_string(),
            "www.example.com".to_string(),
        ),
        ("publisher.location".to_string(), "home".to_string()),
    ]
    .iter()
    .cloned()
    .collect();
